...
def load_trans_data():
    try:
        with open("transation details.json","r") as file:
            return json.load(file)
    except:
        return []
def save_trans_data(tra):
    with open("transation details.json","w") as file:
...
def check_balance():
    ...
    for account in data:
        ...
        print("*" * 30)
def withdraw():
    acc_num=int(input("Enter Clients Account Number to withdraw money:  "))
    for account in data:
        if account["account_number"]==acc_num:
